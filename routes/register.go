...
func cancelRegisteration(context *gin.Context) {
	...
	if err != nil {
		context.JSON(http.StatusInternalServerError, gin.H{"message": "could not cancel registartion"})
		return
	}
	context.JSON(http.StatusCreated, gin.H{"message ": "event was canceled successfully! :)"})
